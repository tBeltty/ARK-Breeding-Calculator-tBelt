import PropTypes from 'prop-types';
import { useTranslation } from 'react-i18next';
import { DataInput, LabelWithTooltip } from '../DataPanel';
import styles from './TroughCalculator.module.css';

/**
 * Food selector section with dropdown and auto-duration toggle.
 */
export function FoodSelectorSection({
    availableFoods,
    selectedFood,
    onSelectedFoodChange,
    desiredHours,
    onDesiredHoursChange,
    isAutoDuration,
    onAutoDurationChange
}) {
    const { t } = useTranslation();

    if (availableFoods.length === 0) return null;

    return (
        <div className={styles.section}>
            <div className={styles.sectionHeader}>
                <LabelWithTooltip
                    label={t('ui.selected_food')}
                    tooltip={t('tooltips.select_food_type')}
                />
                <div style={{ position: 'relative' }}>
                    <select
                        className={styles.select}
                        value={selectedFood || ''}
                        onChange={(e) => onSelectedFoodChange(e.target.value)}
                    >
                        {availableFoods.map(food => (
                            <option key={food} value={food}>
                                {food}
                            </option>
                        ))}
                    </select>
                </div>
            </div>

            {/* Desired Duration Input */}
            <div className={styles.durationRow}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <LabelWithTooltip
                        label={t('ui.desired_duration')}
                        tooltip={t('tooltips.trough_duration')}
                    />
                    {/* Auto Duration Toggle */}
                    <label className={styles.checkboxLabel} style={{ marginLeft: '12px', fontSize: '0.85rem' }}>
                        <input
                            type="checkbox"
                            checked={isAutoDuration}
                            onChange={(e) => onAutoDurationChange(e.target.checked)}
                        />
                        <LabelWithTooltip
                            label={t('ui.auto_duration')}
                            tooltip={t('ui.auto_duration_tooltip')}
                            style={{ marginLeft: '4px', cursor: 'help' }}
                        />
                    </label>
                </div>
                <div className={styles.durationInputWrapper}>
                    <DataInput
                        key={isAutoDuration ? `auto-${desiredHours}` : 'manual'}
                        value={desiredHours}
                        onChange={onDesiredHoursChange}
                        min={0}
                        step={1}
                        placeholder="0"
                        showSteppers={false}
                        hideLabel={true}
                        disabled={isAutoDuration}
                    />
                </div>
            </div>
        </div>
    );
}

FoodSelectorSection.propTypes = {
    availableFoods: PropTypes.arrayOf(PropTypes.string).isRequired,
    selectedFood: PropTypes.string,
    onSelectedFoodChange: PropTypes.func.isRequired,
    desiredHours: PropTypes.number.isRequired,
    onDesiredHoursChange: PropTypes.func.isRequired,
    isAutoDuration: PropTypes.bool.isRequired,
    onAutoDurationChange: PropTypes.func.isRequired
};
